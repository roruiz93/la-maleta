...
// Exportada para que ver-destino.html la pueda llamar
export function initVerDestino() {
  initShared();
  _init();
}

// Auto-init si el script se carga directo (sin import explícito)
window.addEventListener("DOMContentLoaded", () => {
  // Solo corre si el HTML tiene el contenedor correcto y NO fue ya iniciado por el inline script
...
async function _init() {
  ...
  // El HTML usa "ver-destino-contenido"
...
function renderDestino(d, wrap, waNum, waMsg) {
  ...
  const imgPrincipal = d.imagen || imagenes[0] || "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=1400&q=85";

  wrap.innerHTML = `
    <div class="vd-hero">
      <img src="${imgPrincipal}" alt="${d.nombre}">
      ...
      <div class="vd-main">
          ...
          <div class="vd-itinerario">
            ${itinerario.map((dia, i) => `
              <div class="vd-dia">
                ...
                <div class="vd-dia-body"><h3>${dia.titulo || `Día ${i + 1}`}</h3><p>${dia.descripcion || ""}</p></div>
              </div>`).join("")}
          </div>
        </div>` : ""}
        ...
        <div class="vd-section">
          ...
          <div class="vd-galeria">
            ${imagenes.map((src, i) => `<img src="${src}" alt="Foto ${i + 1}" loading="lazy" onclick="abrirFoto('${src}','${d.nombre}')">`).join("")}
          </div>
        </div>` : ""}
        ...
        <div class="vd-section" id="form-consulta">
          ...
          <div class="form-box">
            ...
            <div class="form-field"><label>Teléfono <span style="font-weight:400">(opcional)</span></label>
              <input type="tel" id="vd-tel" placeholder="+54 9 11 0000-0000"></div>
            <div class="form-field"><label>Mensaje</label>
              <textarea id="vd-msg" rows="3">Hola, me interesa el viaje a ${d.nombre}. ¿Podría darme más información?</textarea></div>
  ...
  window.abrirFoto = function(src, alt) {
    ...
    lb.innerHTML = `<div class="lb-bg" onclick="this.parentElement.remove()"></div>
      <img src="${src}" alt="${alt}">
